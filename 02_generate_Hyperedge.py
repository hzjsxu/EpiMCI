...
def build_dict(size, i_list, node2usefulindex, new_data, min_dis, min_freq_cutoff):
	list1 = []
	list1_freq = []
...
def main():  ## add by jsxu
	...
	for k in k_list:
		...
		while jobs_left > 0:
			for i in job_iter:
				process_list.append(pool.submit(build_dict, size, i, node2usefulindex, new_data, min_dis, min_freq_cutoff))
				time.sleep(0.2)
