...
def roc_auc_cuda(y_true, y_pred, size_list, max_size):
    roc_str, aupr_str, precision_str, recall_str, f1_str = "", "", "", "", ""
    fpr_dict, tpr_dict = {}, {}
    ...
    try:
        ...
        roc, aupr = roc_auc_score(
            ...
            y_t, y_p)
        precision = precision_score(y_t_, y_p_)
        recall = recall_score(y_t_, y_p_)
        f1 = f1_score(y_t_, y_p_)

        roc_str += "%s %.3f " % ('all', roc)
        aupr_str += "%s %.3f " % ('all', aupr)
        precision_str += "%s %.3f " % ('all', precision)
        recall_str += "%s %.3f " % ('all', recall)
        f1_str += "%s %.3f " % ('all', f1)

        ## add fpr,tpr
        fpr, tpr, thresholds = roc_curve(y_t, y_p, drop_intermediate=False)
        fpr_dict['all'] = fpr
        tpr_dict['all'] = tpr
        ## add recall precision
        precision_, recall_, _ = precision_recall_curve(y_t, y_p)
        recall_dict['all'] = recall_
        precision_dict['all'] = precision_

        for s in np.unique(size_list):
            y_t = (y_true[size_list == s] > 0.5).float(
            ).cpu().detach().numpy().reshape((-1, 1))
            y_p = y_pred[size_list == s].cpu(
            ...
            precision_str += "%s %.3f " % (str(s), precision)
            recall_str += "%s %.3f " % (str(s), recall)
            f1_str += "%s %.3f " % (str(s), f1)

            fpr, tpr, thresholds = roc_curve(y_t, y_p, drop_intermediate=False)
            fpr_dict[s] = fpr
            tpr_dict[s] = tpr
            precision_, recall_, _ = precision_recall_curve(y_t, y_p)
            recall_dict[s] = recall_
            precision_dict[s] = precision_

        return roc_str[:-1], aupr_str[:-1], precision_str[:-1], recall_str[:-1], f1_str[:-1], fpr_dict, tpr_dict, recall_dict, precision_dict
...
def parallel_build_hash(data, func, initial=None, compress=False, min_size=-1, max_size=-1):
    ...
    for datum in data:
        process_list.append(pool.submit(
            func, datum, compress, min_size, max_size, length))
