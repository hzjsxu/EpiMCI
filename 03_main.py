...
def forward_op_batch_regress(
    ...
    # When label is not generated, prepare the data
    if len(y) == 0:
        x, y, w, s = generate_negative(x, "train_dict", w, neg_num=3)
        x, y, w, s = sync_shuffle([x, y, w, s])
    else:
        s = torch.ones((len(y), 1))
...
def train_epoch(
    ...
    auc1, auc2, precision, recall, f1, fpr_dict, tpr_dict, recall_dict, precision_dict = roc_auc_cuda(y, pred, size_list, max_size)
    acc = accuracy(pred, y, size_list, max_size)

    return bce_total_loss / batch_num, recon_total_loss / batch_num, acc, auc1, auc2, precision, recall, f1, fpr_dict, tpr_dict, recall_dict, precision_dict
...
def eval_epoch(model, loss_func, validation_data, batch_size):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(int(math.floor(len(validation_data) / batch_size))),
            ...
            batch_edge_weight = validation_weight[i *
        ...
        auc1, auc2, precision, recall, f1, fpr_dict, tpr_dict, recall_dict, precision_dict = roc_auc_cuda(label, pred, size_list, max_size)  # return: auc, aupr, fpr, tpr
    return bce_total_loss / (i + 1), recon_total_loss / \
        (i + 1), acc, auc1, auc2, precision, recall, f1, fpr_dict, tpr_dict, recall_dict, precision_dict
...
def train(model,
    ...
    start = time.time()
    final_fpr_dict, final_tpr_dict = {}, {}
    final_recall_dict, final_precision_dict = {}, {}
    for epoch_i in range(epochs):
        ...
        bce_loss, recon_loss, train_accu, auc1, auc2, precision, recall, f1, fpr_dict, tpr_dict, recall_dict, precision_dict = train_epoch(model, loss, training_data_new, optimizer,
                                                                   batch_size)
        ...
        start = time.time()
        valid_bce_loss, recon_loss, valid_accu, valid_auc1, valid_auc2, valid_precision, valid_recall, valid_f1, valid_fpr_dict, valid_tpr_dict, valid_recall_dict, valid_precision_dict = eval_epoch(model, loss, validation_data,
                                                                                    batch_size)
        final_fpr_dict[f'epoch_{epoch_i}'] = valid_fpr_dict
        final_tpr_dict[f'epoch_{epoch_i}'] = valid_tpr_dict
        final_recall_dict[f'epoch_{epoch_i}'] = valid_recall_dict
        final_precision_dict[f'epoch_{epoch_i}'] = valid_precision_dict
        ...
        valid_accus += [valid_aupr_final]  # ?? accu + aupr ?
    ...
    final_fpr_dict['valid'] = valid_fpr_dict
    final_tpr_dict['valid'] = valid_tpr_dict
    final_fpr_dict['valid'] = valid_fpr_dict
    final_tpr_dict['valid'] = valid_tpr_dict
    np.save(os.path.join(temp_dir, 'fpr.npy'), final_fpr_dict)
    np.save(os.path.join(temp_dir, 'tpr.npy'), final_tpr_dict)
    np.save(os.path.join(temp_dir, 'recall.npy'), final_recall_dict)
    np.save(os.path.join(temp_dir, 'precision.npy'), final_precision_dict)
    print(
        '  - (Validation-hyper) bce: {bce_loss: 7.4f}, recon: {recon_loss: 7.4f},'
        '  acc: {accu},'
        ' auc: {auc1}, aupr: {auc2},'
...
def generate_negative(x, dict1, weight="", neg_num=3):
    if len(weight) == 0:
    ...
    if len(neg) == 0:
        neg = torch.zeros(
            (1, pos_part.shape[-1]), dtype=torch.long, device=device)
...
def save_embeddings(model, origin=False):
    ...
    with torch.no_grad():  # 不需要计算梯度
        ids = np.arange(num_list[-1].numpy()) + 1  # 添加.numpy()
...
node2chrom = np.load(os.path.join(
    temp_dir, "node2chrom.npy"), allow_pickle=True).item()
num = []
...
weight_list = []
size_list = [3, 4, 5, 6]
cutoff_list = [12, 4, 3, 3]
for i, size in enumerate(size_list):
    # for size in size_list:
    cutoff = cutoff_list[i]
    data = np.load(os.path.join(temp_dir, "all_%d_counter.npy" %
                   size)).astype('int')
    weight = np.load(os.path.join(
        temp_dir, "all_%d_freq_counter.npy" % size)).astype('float32')
...
embeddings_initial = []
inter_initial = np.load(os.path.join(
    temp_dir, "inter_adj.npy")).astype('float32')
...
#         temp_dir, "all_%d_freq_counter.npy" % size)).astype('float32')
#     print("before filter", "size", size, "length", len(data))
#     weight = QuantileTransformer(n_quantiles=1000, output_distribution='uniform').fit_transform(
#         weight.reshape((-1, 1))).reshape((-1))
...
optimizer = torch.optim.AdamW(params_list, lr=1e-5, amsgrad=False)
alpha = 0.5
beta = 0.01
...
train(classifier_model,
      ...
      training_data=(train_data, train_weight),
      validation_data=(test_data, test_weight),
      optimizer=[optimizer], epochs=50, batch_size=batch_size)
